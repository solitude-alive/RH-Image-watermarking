...
def save_images(original_images, watermarked_images, epoch, folder, resize_to=None):
    ...
    filename = os.path.join(folder, 'encoder-epoch-{}.png'.format(epoch))
...
def save_noised_images(encoded_images, noised_images, epoch, folder, resize_to=None):
    images = encoded_images[:encoded_images.shape[0], :, :, :].cpu()
    noised_images = noised_images[:noised_images.shape[0], :, :, :].cpu()

    # scalevalues to range [0, 1] from original range of [-1, 1]
    images = (images + 1) / 2
    ...
    stacked_images = torch.cat([images, noised_images], dim=0)
    filename = os.path.join(folder, 'noised-epoch-{}.png'.format(epoch))
    # torchvision.utils.save_image(stacked_images, filename, original_images.shape[0], normalize=False)
    torchvision.utils.save_image(stacked_images, filename)


def save_secret_images(original_secret_images, decoded_secret_images, epoch, folder, resize_to=None):
    images = original_secret_images[:original_secret_images.shape[0], :, :, :].cpu()
    decoded_secret_images = decoded_secret_images[:decoded_secret_images.shape[0], :, :, :].cpu()
    ...
    if resize_to is not None:
        images = F.interpolate(images, size=resize_to)
        decoded_secret_images = F.interpolate(decoded_secret_images, size=resize_to)

    stacked_images = torch.cat([images, decoded_secret_images], dim=0)
    filename = os.path.join(folder, 'decoder-epoch-{}.png'.format(epoch))
    # torchvision.utils.save_image(stacked_images, filename, original_images.shape[0], normalize=False)
    torchvision.utils.save_image(stacked_images, filename)


def save_cover_images(original_cover_images, decoded_cover_images, epoch, folder, resize_to=None):
    images = original_cover_images[:original_cover_images.shape[0], :, :, :].cpu()
    decoded_cover_images = decoded_cover_images[:decoded_cover_images.shape[0], :, :, :].cpu()

    # scale values to range [0, 1] from original range of [-1, 1]
    images = (images + 1) / 2
    decoded_cover_images = (decoded_cover_images + 1) / 2

    if resize_to is not None:
        images = F.interpolate(images, size=resize_to)
        decoded_cover_images = F.interpolate(decoded_cover_images, size=resize_to)

    stacked_images = torch.cat([images, decoded_cover_images], dim=0)
    filename = os.path.join(folder, 'cover-epoch-{}.png'.format(epoch))
    # torchvision.utils.save_image(stacked_images, filename, original_images.shape[0], normalize=False)
    torchvision.utils.save_image(stacked_images, filename)


def train_save_images(original_images, watermarked_images, epoch, folder, resize_to=None):
    images = original_images[:original_images.shape[0], :, :, :].cpu()
    watermarked_images = watermarked_images[:watermarked_images.shape[0], :, :, :].cpu()
    ...
    stacked_images = torch.cat([images, watermarked_images], dim=0)
    filename = os.path.join(folder, 'train-encoder-epoch-{}.png'.format(epoch))
    # torchvision.utils.save_image(stacked_images, filename, original_images.shape[0], normalize=False)
    torchvision.utils.save_image(stacked_images, filename)


def train_save_noised_images(encoded_images, noised_images, epoch, folder, resize_to=None):
    images = encoded_images[:encoded_images.shape[0], :, :, :].cpu()
    noised_images = noised_images[:noised_images.shape[0], :, :, :].cpu()
    ...
    stacked_images = torch.cat([images, noised_images], dim=0)
    filename = os.path.join(folder, 'train-noised-epoch-{}.png'.format(epoch))
    # torchvision.utils.save_image(stacked_images, filename, original_images.shape[0], normalize=False)
    torchvision.utils.save_image(stacked_images, filename)


def train_save_secret_images(original_secret_images, decoded_secret_images, epoch, folder, resize_to=None):
    images = original_secret_images[:original_secret_images.shape[0], :, :, :].cpu()
    decoded_secret_images = decoded_secret_images[:decoded_secret_images.shape[0], :, :, :].cpu()
    ...
    stacked_images = torch.cat([images, decoded_secret_images], dim=0)
    filename = os.path.join(folder, 'train-decoder-epoch-{}.png'.format(epoch))
    # torchvision.utils.save_image(stacked_images, filename, original_images.shape[0], normalize=False)
    torchvision.utils.save_image(stacked_images, filename)


def save_residual_images(original_images, residual_images, epoch, folder, resize_to=None):
    images = original_images[:original_images.shape[0], :, :, :].cpu()
    out_images = residual_images[:residual_images.shape[0], :, :, :].cpu()

    # scale values to range [0, 1] from original range of [-1, 1]
    images = (images + 1) / 2
    out_images = (out_images + 1) / 2

    if resize_to is not None:
        images = F.interpolate(images, size=resize_to)
        out_images = F.interpolate(out_images, size=resize_to)

    stacked_images = torch.cat([images, out_images], dim=0)
    filename = os.path.join(folder, 'residual-epoch-{}.png'.format(epoch))
    # torchvision.utils.save_image(stacked_images, filename, original_images.shape[0], normalize=False)
    torchvision.utils.save_image(stacked_images, filename)
...
def save_checkpoint(model: Hidden, experiment_name: str, epoch: int, checkpoint_folder: str):
    ...
    checkpoint = {
        ...
        'cnn-f-model  ': model.cnn_f.state_dict(),
        'enc-dec-cnnf-optim': model.optimizer_enc_dec_cnn.state_dict(),
...
def model_from_checkpoint(hidden_net, checkpoint):
    ...
    hidden_net.cnn_f.load_state_dict(checkpoint['cnn-f-model  '])
    hidden_net.optimizer_enc_dec_cnn.load_state_dict(checkpoint['enc-dec-cnnf-optim'])
...
def load_options(options_file_name) -> (TrainingOptions, NetConfiguration, dict):
...
def message_to_image(messages, repeat_times=4, rows=128, device=torch.device):
    im = []
    for batch_me in messages:
        ...
        for me in batch_me:
            if me == 0:
                me = me - 1
            new = me.tolist()
            new = [new] * repeat_times * rows
            new = np.array(new)
            new = new.reshape((rows, repeat_times))
            batch_im.append(new)
        batch_im = np.concatenate(batch_im, axis=1)
        im.append(batch_im)
